...
// Planning patron : agenda type calendrier (Mois / Semaine / Jour) croisant les
...
export default function PlanningPage() {
  ...
  useEffect(() => {
    ...
    const url =
      view === "mois"
        ? `/api/admin/planning?month=${ymKey(refDate)}`
        : `/api/admin/planning?from=${days[0]}&to=${days[days.length - 1]}`;
    fetch(url)
      .then((r) => {
        if (r.status === 401) {
  ...
  useEffect(() => {
    fetch(`/api/admin/planning?from=${todayKey}&to=${todayKey}`)
